...
df_clean <- 
  ...
  mutate(
    ...
    h_income=factor(case_when(
      ...
      h_income=="150,000 - <200,000"~"150 - <200", 
      TRUE~"200+"), 
      levels=c("<50", "100 - <150", "150 - <200", "200+")),
...
source(paste0(function.dir, "trend_het_test.R"))
source(paste0(function.dir, "order_row.R"))
source(paste0(function.dir, "add_table_header.R"))
...
# Data manipulation: Ensure sufficient number of observations per category
df_clean <- 
  df_clean %>% 
  mutate(
    edu_recode=factor(case_when(education=="No formal"|education=="Primary"~"No formal or Primary", 
                                TRUE~education),
...
vars_list=list(
  "age10"=c("gender", "health_area", "m_status", "edu_recode", "employment_status", "h_income", "chronic_d"), 
  "gender"=c("age_years", "I(age_years^2)", "health_area", "m_status", "edu_recode", "employment_status", "h_income", "chronic_d"),
  "m_status"=c("age_years", "I(age_years^2)", "gender", "health_area",  "edu_recode", "employment_status", "h_income", "chronic_d"), 
  # "health_area"=c("age_years", "I(age_years^2)",  "gender", "m_status", "edu_recode", "employment_status", "h_income", "chronic_d"),
  "edu_recode"=c("age_years", "I(age_years^2)", "gender", "health_area", "m_status", "employment_status", "h_income", "chronic_d"),
  "employment_status"=c("age_years", "I(age_years^2)", "gender", "health_area", "m_status", "edu_recode", "h_income", "chronic_d"),
  "h_income"=c("age_years", "I(age_years^2)", "gender", "health_area", "m_status", "employment_status", "edu_recode", "chronic_d"),
  "chronic_d"=c("age_years", "I(age_years^2)", "gender", "health_area", "m_status", "employment_status", "edu_recode", "h_income"),
  "parent_vax_any"=c("age_years", "I(age_years^2)", "gender", "health_area", "m_status", "employment_status", "edu_recode", "h_income", "chronic_d")
)
...
for (oo in vars_outcome) {
  ...
  # Continue here: Get floating absolute risk and plot and results 
}
...
walk2(.x=forest.table, 
      .y=c("Regression HPV.csv", "Regression Cancer.csv"), 
      ~write.csv(.x, file=.y))
...
for (k in names(df_reg)) {
  shape_data[[k]] <- map(.x=c("age10", "edu_recode", "h_income"), ~df_reg[[k]][[.x]])
}
...
SetPage(orient ="LANDSCAPE", 
        ...
        suppress.date = TRUE,
        titlespace=0.02,
#         footer="All analyses were stratified by age-at-risk, sex, study area and smoking status and adjusted for income, education, occupation, \noutdoor ambient temperature, alcohol, sedentary time, physical activity, heart rate, body mass index, diabetes, ephysema, bronchitis and COPD
# 
# The numbers above the confidence intervals (CIs, vertical line) are the adjusted HRs
# The numbers below the CIs are the number of event (adjusted incidence rate per 10\u00B3 person-years)\n \n
#         
# ",
        footer.cex = 1.3,
        footerspace=0.08,
